...
export default function SettingsPage() {
  ...
  const handleChangeTags = (arg: any, index: number) => {
    if (arg === "Theme") {
      setTags("Theme");
    }
    if (arg === "Profile") {
  ...
  return (
    <div className="container-xl px-4 mt-4">
      <nav className="nav nav-borders">
        {settingTags.map((v: any, index: any) => (
          <>
            <div
              onClick={() => handleChangeTags(v.title, index)}
              className={`nav-link  ms-0 ${activeTag.index===index && activeTag.show ? "active" : ""} `}
