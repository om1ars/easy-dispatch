...
export default function SettingsPage() {
  ...
  const [activeTag, setActiveTag] = useState([false, false, false, false]);

  const handleChangeTags = (arg: any, i:number) => {
    if (arg === "Theme") {
      setTags("Theme");
    }
    if (arg === "Profile") {
    ...
    if (arg === "Notifications") {
      setTags("You don't have any notifications yet");
    }
    const resultOfActiveClass = [activeTag.slice(0, i), !activeTag[i], activeTag.slice(index+1)]
  };
    ...
    <div className="container-xl px-4 mt-4">
      <nav className="nav nav-borders">
        {settingTags.map((v: any, i: any) => (
          <>
            <div
              onClick={() => handleChangeTags(v.title, i)}
              className={`nav-link  ms-0 ${activeTag ? "active" : ""} `}
